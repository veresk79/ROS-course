...
void poseCallback(const visualization_msgs::MarkerConstPtr& msg){
 static tf::TransformBroadcaster br;
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "r2"));
...
int main(int argc, char** argv){
  ros::init(argc, argv, "broadcaster2");
 ros::NodeHandle node;
 ros::Subscriber sub = node.subscribe("visualization_marker2", 1000, &poseCallback);
 ros::spin();
